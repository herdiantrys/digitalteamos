    ...
    return (
        <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
                {tab === 'export' ? <ExportTab /> : <ImportWizard />}
            </div>
        </div>
    );
}
...
function ExportTab() {
    ...
    const handleCSV = async () => { setLoading(true); try { downloadFile(await exportContentCSV(), `content-${Date.now()}.csv`, 'text/csv;charset=utf-8;'); } finally { setLoading(false); } };
    const handleMD = async () => { setLoading(true); try { downloadFile(await exportContentMarkdown(), `content-${Date.now()}.md`, 'text/markdown'); } finally { setLoading(false); } };
...
function ImportWizard() {
    ...
    const handleFile = async (file: File) => {
        ...
        if (md) {
            // For markdown, skip analyze step — go straight to quick import
            setStep('review');
            setAnalysis(null);
            return;
        }
        ...
        setLoading(true);
        try {
            const res = await analyzeImportCSV(text);
            setAnalysis(res);
            setColMappings(res.columnMappings);
            setStep('review');
    ...
    if (step === 'upload') return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                Upload a <strong>CSV</strong> or <strong>.md</strong> file. Columns are auto-matched to properties. <strong>Conflicts</strong> (duplicate titles) will be detected before importing.
            </div>
            ...
            <div style={{ background: 'var(--sidebar-bg)', borderRadius: 8, padding: 12, fontSize: 11, color: 'var(--text-secondary)', border: '1px solid var(--border-color)' }}>
                <div><strong style={{ color: 'var(--text-primary)' }}>CSV:</strong> <code>title,platform,status,PropertyName</code></div>
                <div style={{ marginTop: 4 }}><strong style={{ color: 'var(--text-primary)' }}>Markdown:</strong> YAML frontmatter with <code>title:</code>, <code>platform:</code>, <code>status:</code> + property keys</div>
            </div>
        </div>
    );
    ...
    if (step === 'review') {
        ...
        const handleTypeChange = (header: string, type: string) => {
            setColMappings(prev => prev.map(m => m.header === header ? { ...m, detectedType: type } : m));
        };

        const handlePropChange = (header: string, propId: string) => {
            setColMappings(prev => prev.map(m => {
                if (m.header !== header) return m;
                if (propId === 'NEW') return { ...m, existingPropId: undefined, isNew: true };
                const found = analysis?.existingProperties.find(p => p.id === propId);
                return { ...m, existingPropId: propId, isNew: false, detectedType: found?.type || m.detectedType };
            }));
        };
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {analysis && colMappings.length > 0 && (
                    <div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em', display: 'flex', justifyContent: 'space-between' }}>
                            <span>Column Mapping</span>
                            <span style={{ fontWeight: 400, opacity: 0.7 }}>Check "Include" to import column data</span>
                        </div>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, minWidth: 500 }}>
                                    <thead style={{ background: 'rgba(255,255,255,0.02)' }}>
                                        <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                                            <th style={{ ...th, width: 40, textAlign: 'center' }}>In</th>
                                    ...
                                    <tbody>
                                        {colMappings.map((m, i) => (
                                            <tr key={m.header} style={{ borderBottom: i < colMappings.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                                                ...
                                                <td style={td}>
                                                    <div style={{ fontWeight: 600, fontSize: 11, color: 'var(--text-primary)', marginBottom: 2 }}>{m.header}</div>
                                                    <div style={{ fontSize: 10, color: 'var(--text-secondary)', opacity: 0.7, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 140 }}>
                                                        {m.sampleValue || '(empty)'}
                                                    </div>
                                                </td>
                                                <td style={td}>
                                                    {m.isBuiltIn ? (
                                                        ...
                                                        <select
                                                            value={m.existingPropId || 'NEW'}
                                                            onChange={e => handlePropChange(m.header, e.target.value)}
                                                            style={{
                                                                width: '100%', padding: '4px 6px', fontSize: 11, borderRadius: 6,
                                                ...
                                                <td style={td}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                                        <select
                                                            ...
                                                            onChange={e => handleTypeChange(m.header, e.target.value)}
                                                            disabled={m.isBuiltIn}
                                                            style={{
                                                                padding: '4px 6px', fontSize: 11, borderRadius: 6,
                                                                border: '1px solid var(--border-color)', background: 'var(--bg-color)',
                                                                color: 'var(--text-primary)', outline: 'none',
                                                                opacity: m.isBuiltIn ? 0.5 : 1
                                                            }}
                                                        >
                ...
                {conflictCount > 0 && (
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            {(['skip', 'replace', 'add'] as ConflictResolution[]).map(r => (
                                <button key={r} onClick={() => setGlobalRes(r)} style={{
                                    ...
                                    fontWeight: globalRes === r ? 700 : 400,
                        ...
                        {/* Per-conflict list */}
                        <div style={{ background: 'var(--sidebar-bg)', borderRadius: 8, border: '1px solid var(--border-color)', maxHeight: 240, overflowY: 'auto' }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 6 }}>
                            <strong>Skip</strong> = ignore · <strong>Replace</strong> = overwrite existing · <strong>Add</strong> = create duplicate
                        </div>
                    </div>
                )}
