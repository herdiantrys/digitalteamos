...
import { Type, Hash, ChevronDown, Layers, Calendar, User, CheckSquare, Link as LinkIcon, AtSign, Phone, X, Trash2, ArrowDown, ArrowUp, ArrowUpDown, Check, Edit, Save } from 'lucide-react';
...
export default function TableView({
    ...
    const visibleProps = useMemo(() => {
        ...
        try { return JSON.parse(viewSettings.propertyVisibility) as string[]; } catch { return null; }
    }, [viewSettings?.propertyVisibility]);

    // Filter properties based on visibility settings
    const displayProperties = useMemo(() => {
        if (!visibleProps) return properties;
        return properties.filter(p => visibleProps.includes(p.id));
            ...
            <div className="tv-mobile">
                {pagedGrouped.map(group => (
                    <div key={group.name} style={{ marginBottom: 20 }}>
                        ...
                        {group.items.map(c => <MobileCard key={c.id} content={c} properties={orderedProps} userOptionsRaw={userOptionsRaw} selected={selectedIds.has(c.id)} onToggle={() => toggleRow(c.id)} currentUser={currentUser} onPermissionDenied={() => setShowPermissionWarning(true)} />)}
                    </div>
                ))}
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', tableLayout: 'fixed' }}>
                        <thead style={{ position: 'sticky', top: 0, zIndex: 15, background: '#fafafa', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                            <tr style={{ borderBottom: '1px solid rgba(0, 0, 0, 0.06)' }}>
                                ...
                                {orderedProps.map((p, idx) => {
                                    ...
                                    return (
                                        <th
                                            ...
                                            className={`th-draggable ${dropIdx === idx ? (draggingIdx !== null && draggingIdx < idx ? 'drop-indicator-right' : 'drop-indicator-left') : ''}`}
                                            style={{ padding: '14px 16px', width: w, minWidth: MIN_COL_WIDTH, position: 'relative', borderLeft: '1px solid rgba(0,0,0,0.03)', boxSizing: 'border-box' }}
                                        >
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                                                <span style={{ display: 'flex', flexShrink: 0 }}>{getTypeIcon(p.type)}</span>
                                ...
                                {/* Actions Column */}
                                <th style={{ padding: '14px 16px', width: 60, boxSizing: 'border-box' }}>
                                    {/* Empty filler header for sticky background */}
                                </th>
                                ...
                                <Fragment key={group.name}>
                                    {group.name && (
                                        <tr style={{ background: '#f8f9fa' }}>
                                            <td colSpan={orderedProps.length + 4} style={{ padding: '8px 16px', fontWeight: 700, fontSize: 11, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    {group.name}
                                    ...
                                    {group.items.map((content) => {
                                        ...
                                        return (
                                            <tr key={content.id} className={`tv-row${isSelected ? ' tv-row-sel' : ''}`}
                                                ...
                                                <td style={{ padding: '14px 16px', textAlign: 'right', boxSizing: 'border-box' }}>
                                                    <DeleteButton
                                                        contentId={content.id}
