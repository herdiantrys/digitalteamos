...
import { Table, Kanban, LayoutGrid, List as ListIcon, Clock, Calendar, Search, SlidersHorizontal, X, ChevronDown, Plus, MoreHorizontal } from 'lucide-react';
...
export default function ContentViewer({
    ...
    const handleCreateView = async () => {
        ...
        try {
            ...
            const newView = await createContentView({
                name: 'New View',
                layout: 'table',
                order: newOrder,
            });
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {/* View Switching Tabs */}
            <div style={{ display: 'flex', gap: 4, paddingBottom: 8, overflowX: 'auto', whiteSpace: 'nowrap', borderBottom: '1px solid var(--border-color)', alignItems: 'center' }}>
                {views.map(view => (
                    <div key={view.id} style={{ position: 'relative' }}>
                        ...
                        {activeViewId === view.id && currentUser?.role === 'ADMIN' && (
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                    ...
                    <>
                        ...
                        <div style={{
                            ...
                            <ViewSettingsMenu
                                ...
                                onUpdate={(updated) => handleUpdateViewLocally(openView.id, updated)}
                            />
                        </div>
                    </>,
                    document.body
